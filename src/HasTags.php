...
trait HasTags
{
    ...
    {
        return $this->morphToMany(self::getTagClassName(), 'taggable')->ordered();
    }
    /**
     ...
     */
    public function scopeWithAllTags(Builder $query, $tags, $type = null)
    {
        $tags = static::convertToTags($tags, $type);
        collect($tags)->each(function ($tag) use ($query) {
            $query->whereIn("{$this->getTable()}.{$this->getKeyName()}", function ($query) use ($tag) {
                $query->from('taggables')->select('taggables.taggable_id')->where('taggables.tag_id', $tag ? $tag->id : 0);
            });
        });
        return $query;
    }
     ...
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function scopeWithAnyTags(Builder $query, $tags, $type = null)
    {
        $tags = static::convertToTags($tags, $type);
    ...
    public function scopeWithAllTagsOfAnyType(Builder $query, $tags)
    {
    ...
    public function scopeWithAnyTagsOfAnyType(Builder $query, $tags)
    {
        $tags = static::convertToTagsOfAnyType($tags);
        return $query->whereHas('tags', function (Builder $query) use ($tags) {
            $tagIds = collect($tags)->pluck('id');
    ...
    public function tagsWithType($type = null)
    {
    ...
    public function detachTags($tags)
    {
        ...
        collect($tags)->filter()->each(function (Tag $tag) {
     ...
     * @return $this
     */
    public function syncTagsWithType($tags, $type = null)
    {
        $className = static::getTagClassName();
     ...
     */
    protected function syncTagIds($ids, $type = null, $detaching = true)
    {
        $isUpdated = false;
        // Get a list of tag_ids for all current tags
        $current = $this->tags()->newPivotStatement()->where('taggable_id', $this->getKey())->where('taggable_type', $this->getMorphClass())->when($type !== null, function ($query) use ($type) {
            $tagModel = $this->tags()->getRelated();
            return $query->join($tagModel->getTable(), 'taggables.tag_id', '=', $tagModel->getTable() . '.' . $tagModel->getKeyName())->where('tags.type', $type);
        })->pluck('tag_id')->all();
